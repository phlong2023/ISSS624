...
The R-Squared value indicates that the model can account for roughly 36.17% of the variation in the number of trips between each pair of origin-destination.
...
Similar to the Unconstrained Model, it can be seen that all p-value of each factor is much lower than 0.05, showing that they are all statistically significant. However, a cursory examination show that the coefficients values are lower than in the Unconstrained Model.
...
The R-Squared value indicates that the model can account for roughly 25.37% of the variation in the number of trips between each pair of origin-destination.
...
The R-Squared value indicates that the model can account for roughly 23.2% of the variation in the number of trips between each pair of origin-destination.
...
The R-Squared value indicates that the model can account for roughly 12.23% of the variation in the number of trips between each pair of origin-destination.
...
From this Model Comparison, the Unconstrained Model as the lowest RMSE, indicating that its fitted values is the closest to the actual values compared to the other models. Additionally, the Akaike Information Criterion (AIC), which tends to prefer less complex models with better explanatory power, tells us that the Unconstrained Model is the most appropriate due to it having the lowest value.

Thus, the Unconstrained Model would be the most appropriate model for our purpose. Before moving on to examine it in further depth, the fitted values can be visualized.
...
From a brief examination, it is identifiable that the Unconstrained Model provides the best fit line between the predicted values and the actual values.

### Analyzing Chosen Model

The coefficient values of the Unconstrained Model can be displayed again for further analysis of travel during the Weekend/Holiday Morning Peak 11am - 2pm (11:00 - 14:00).

```{r}
summary(uncSIM)
```

From this transformed value, it is possible to conclude the following:

-   ORIG.Entertainment: Every additional Entertainment venue at the origin hexagon cell would **increase** the log number of trips by roughly 0.02.

-   ORIG.Retails: Every additional Retail venue at the origin hexagon cell would **decrease** the log number of trips by roughly 0.043.

-   ORIG.Leisure: Every additional Leisure venue at the origin hexagon cell would **decrease** the log number of trips by roughly 0.176.

-   ORIG.FinServ: Every additional Financial Service venue at the origin hexagon cell would **increase** the log number of trips by roughly 0.398.

-   ORIG.FoodBev: Every additional F&B venue at the origin hexagon cell would **decrease** the log number of trips by roughly 0.068.

-   ORIG.trainstation: Every additional train station at the origin hexagon cell would **decrease** the log number of trips by roughly 0.046.

-   ORIG.busstop: Every additional bus stop at the origin hexagon cell would **increase** the log number of trips by roughly 0.3511.

-   ORIG.capacity: Every additional potential resident at the origin hexagon cell would **increase** the log number of trips by roughly 0.0752.

-   DESTIN.Entertainment: Every additional Entertainment venue at the destination hexagon cell would **increase** the log number of trips by roughly 0.0242.

-   DESTIN.Retails: Every additional Retail venue at the destination hexagon cell would **decrease** the log number of trips by roughly 0.0377.

-   DESTIN.Leisure: Every additional Leisure venue at the destination hexagon cell would **decrease** the log number of trips by roughly 0.1703.

-   DESTIN.FinServ: Every additional Financial Service venue at the destination hexagon cell would **increase** the log number of trips by roughly 0.5634.

-   DESTIN.FoodBev: Every additional F&B venue at the destination hexagon cell would **decrease** the log number of trips by roughly 0.1077.

-   DESTIN.trainstation: Every additional train station at the destination hexagon cell would **increase** the log number of trips by roughly 0.0216.

-   DESTIN.busstop: Every additional bus stop at the destination hexagon cell would **increase** the log number of trips by roughly 0.4347.

-   dists: Every additional unit of distance between the origin and the destination hexagon cell would **decrease** the log number of trips by roughly 1.4787.

From the coefficients above, there are some expected as well as unexpected results.

The result that every additional unit of distance would decrease the number of inter-zonal bus trips is self-explanatory, as longer distance would make travel less desirable, especially during the Weekend/Holiday Morning Peak. Similarly, the positive relationship between the destination number of bus stop, train station, and entertainment venues are intuitive. Having more bus stop and train station make a location more accessible while entertainment venues make a location more attractive; these would make them into attractive factors.

On the propulsive factors end, the negative relationships between the origin number of retail, leisure, and F&B venues and number of inter-zonal trips indicate that the availability of these venues would decrease the likelihood of inter-zonal trips; people are less likely to commit to inter-zonal travel on a weekend/holiday morning if their needs can be met locally. Similarly, the positive relationship between the origin number of bus stop and number of inter-zonal bus trips can be understood as the accessibility of the travel option under consideration. Additionally, the positive relationship between the origin potential number of residents and the number of inter-zonal bus trips can possibly be understood as increasing the number of potential commuters.

However, there are results which do not seem to fit intuitively into the conventional understanding of travel. In the origin cell, the positive relationship between the number of entertainment and financial service venues with number of inter-zonal bus trips is harder to explain. It is not clear why having more of these venues would increase the number of trips. Similarly, in the destination cell, the negative relationship between the number of retails, leisure, and F&B venues and the number of inter-zonal bus trips are confounding. It is not intuitive why having more of these potentially attractive venues would decrease the number of trips.

Overall, even though the Unconstrained Model provides the best fitted values for the number of inter-zonal bus trips between hexagon cells, there are still unexplained effects in the spatial interaction model. Some of the coefficient values for produced by the models make intuitive sense while others are harder to explain. Additionally, the R-Squared value 36.16% indicates that the percentage of variation in the number of inter-zonal bus trips explained by the model remain low. Thus, further improvements in future research to improve the explanatory power of the model can be made.
